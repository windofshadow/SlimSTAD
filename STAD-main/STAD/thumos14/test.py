...
if __name__ == '__main__':
    ...
    for i in range(28, 29):
        print(i)
        checkpoint_path = "/home/lzdjohn/AFSD/AFSDtime/AFSD/thumos14/models/thumos14/checkpoint-"+str(i)+".ckpt"
        json_name = "/home/lzdjohn/AFSD/AFSDtime/AFSD/thumos14/output/"+str(i)+".json"
        ...
        if fusion:
            ...
            net = BDNet(in_channels=1,
                        backbone_model="/home/lzdjohn/AFSD/AFSDtime/models/thumos14/checkpoint-15.ckpt",
                        training=False)
            # net = nn.DataParallel(net, device_ids=list(range(2))).cuda()
        ...
        for video_name in tqdm.tqdm(list(video_infos.keys()), ncols=0):
            ...
            # # data = np.expand_dims(data, 0).repeat(3, axis=0)
            # data = torch.from_numpy(data).float().unsqueeze(2)  # 1*8500*1*30
            # print(data.shape)
            # # data = data.view(3, 340, 25, 30)
